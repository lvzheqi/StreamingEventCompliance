...
from simulate_stream_event.client_logging import Client_logging
...
class Client_cls(object):
    ...
    def run_compliance_checker(self):
        ...
        try:
            Client_logging().log_info(func_name, self.uuid, "Calling read_log()")
    ...
    def run_show_deviation_pdf(self):
        func_name = sys._getframe().f_code.co_name
        try:
            Client_logging().log_info(func_name, self.uuid, "Post request to http://127.0.0.1:5000/show-deviation-pdf?uuid="+ self.uuid)
            r = requests.post('http://127.0.0.1:5000/show-deviation-pdf?uuid=' + self.uuid)
            if r.status_code != 200:
                Client_logging().log_error(func_name, self.uuid, "pdf can not be created")
                print('Error: pdf can not be created')
            else:
                if r.text: # TODO: according to the status of server
                    Client_logging().log_info(func_name, self.uuid, "Compliance checking is done. Deviations PDF is "
                    ...
                    Client_logging().log_error(func_name, self.uuid, self.uuid +
            ...
            Client_logging().log_error(func_name, self.uuid, "Error: The server is not available, please try it later!")
            print(ConnectionException.message)
            return
...
def main(argv):
    ...
    try:
        ...
        Client_logging().log_info(func_name, argv[0], "Post request to server:http://127.0.0.1:5000/login?uuid="+argv[0])
        r = requests.post('http://127.0.0.1:5000/login?uuid=' + argv[0])
        if r.status_code != 200:
            print('Error: The user can not be created!')
            Client_logging().log_error(func_name, argv[0], "The user can not be created!")
            return

    except requests.exceptions.RequestException:
        Client_logging().log_error(func_name, argv[0], "The server is not available, please try it later!")
        print(ConnectionException.message)
        return

    if len(argv) > 1 and not os.path.exists(argv[1]):
        Client_logging().log_error(func_name, argv[0], "The given path is not available!")
    ...
    while(True):
        Client_logging().log_info(func_name, argv[0], "The options are displayed")
        ...
        if services == '1':
            Client_logging().log_info(func_name, argv[0], "The user selected option 1")

            Client_logging().log_info(func_name, argv[0], "Calling run_compliance_checker()")
            print('---------------start to do compliance checking, please wait-------------------')
            try:
                p_main = Process(target=client1.run_compliance_checker())
                p_main.start()
                p_main.join()
                Client_logging().log_info(func_name, argv[0], "compliance checking is completed")
                print('------------------the compliance checking is finishing------------------------')
            except ReadFileException:
                Client_logging().log_error(func_name, argv[0], "Input file is not readable!")
                print(ReadFileException.message)
                ...
                Client_logging().log_error(func_name, argv[0], "Compliance checking is interrupted by user")
                print('------------------the compliance checking is interrupt------------------------')
        elif services == '2':
            Client_logging().log_info(func_name, argv[0], "The user selected option 2")
            ...
            print('Bye!')
            Client_logging().log_info(func_name, argv[0], "Exiting...")
            return
        else:
            Client_logging().log_error(func_name, argv[0], "The users input is invalid")
