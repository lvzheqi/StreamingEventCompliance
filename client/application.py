...
class Client_cls(object):
    ...
    def run_compliance_checker(self):
        ...
        try:
            client_logging.log_info(func_name, username=self.uuid, message="Calling read_log()")
            event_log = eventlog.read_log(self.uuid, self.path)
        except Exception:
            client_logging.log_error(func_name=func_name, username=self.uuid, message="Exception raised while reading file")
            raise ReadFileException
        client_logging.log_info(func_name=func_name, username=self.uuid, message="Calling simulate_stream_event()")
    ...
    def run_show_deviation_pdf(self):
        ...
        try:
            ...
            if r.status_code != 200:
                client_logging.log_error(func_name=func_name, username=self.uuid, message="pdf can not be created")
...
def main(argv):
    ...
    if len(argv) < 2 or len(argv) > 3:
        ...
        client_logging.log_error(func_name=func_name, message="Username or Event logger path arguments"
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            client_logging.log_error(func_name=func_name, username=argv[0], message="The user can not be created!")
            return
    except Exception:
    ...
    if not os.path.exists(argv[1]):
        client_logging.log_error(func_name=func_name, username=argv[0], message="The given path is not available!")
    ...
    while(True):
        client_logging.log_info(func_name=func_name, username=argv[0], message="The options are displayed")
        ...
        if services == '1':
            client_logging.log_info(func_name=func_name, username=argv[0], message="The user selected option 1")

            client_logging.log_info(func_name=func_name, username=argv[0], message="Calling run_compliance_checker()")
            print('---------------start to do compliance checking, please wait-------------------')
            try:
                p_main = Process(target=client1.run_compliance_checker())
                p_main.start()
                p_main.join()
                client_logging.log_info(func_name=func_name, username=argv[0], message="compliance checking is completed")
                print('------------------the compliance checking is finishing------------------------')
            except ReadFileException:
                client_logging.log_error(func_name=func_name, username=argv[0], message="Input file is not readable!")
                print('Error: The input file is not readable!')
                print('------------------the compliance checking is interrupt------------------------')
            except KeyboardInterrupt:
                client_logging.log_error(func_name=func_name, username=argv[0], message="Compliance checking is interrupted by user")
                print('------------------the compliance checking is interrupt------------------------')
        elif services == '2':
            client_logging.log_info(func_name=func_name, username=argv[0], message="The user selected option 2")
            print('-----------------start to render deviation pdf, please wait-------------------')
            client_logging.log_info(func_name=func_name, username=argv[0], message="Calling  run_show_deviation_pdf() ")
            client1.run_show_deviation_pdf()
            print('------------------------------------------------------------------------------')
        elif services == '3':
            client_logging.log_info(func_name=func_name, username=argv[0], message="The user selected option 3")
            print('Bye!')
            client_logging.log_info(func_name=func_name, username=argv[0], message="Exiting...")
            return
        else:
            client_logging.log_error(func_name=func_name, username=argv[0], message="The users input is invalid")
