...
class AlertlogTest(unittest.TestCase):
    ...
    def setUp(self):
        self.uuid = 'u1'
        self.alog1 = alertlog.AlertLog()
        self.alog2 = alertlog.AlertLog()
    ...
    def test_alertlog(self):
        for alog in self.alog1._alert_log.values():
            if alog.sink_node is 'B':
                self.assertEqual(alog.get_alert_count(), 1)
            elif alog.sink_node is 'C':
                self.assertEqual(alog.get_alert_count(), 1)
            elif alog.sink_node is '$':
                self.assertEqual(alog.get_alert_count(), 0)
        for alog in self.alog2._alert_log.values():
            if alog.sink_node is 'B,D':
                self.assertEqual(alog.get_alert_count(), 2)
