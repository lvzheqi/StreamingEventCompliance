...
def run_build(event):
    ...
    try:
        while C.lock_List.get(event['case_id']) != event['iterator']:
            # print("while")
            ServerLogging().log_info(func_name, "server", event['case_id'],
                                     event['activity'], "waiting for " + str(C.lock_List.get(event['case_id']))
                                     + " equal to event['iterator']: " + str(event['iterator']))
            pass
        # if C.lock_List.get(event['case_id']) == event['iterator']:
        #     print(event['case_id'], "before while", C.lock_List.get(event['case_id']), event['iterator'])
        ...
        C.dictionary_cases.get(event['case_id']).pop(event['iterator'] - MAXIMUN_WINDOW_SIZE)
